...
def convert_osm_to_gpkg(input_filename: str, tmp_filename: str, layer_name: str):
    ...
    subprocess.run(
        # For S602 error - see Jira issue DATAPIPE-89 for more information
        command.format(input=input_filename, output=tmp_filename),
...
def convert_osm2gpkg(in_file, out_file, geom_type="multipolygons"):
    ...
    out_lyr = out_ds.CopyLayer(
        in_lyr, geom_type, options=["OVERWRITE=YES", "ENCODING=UTF-8"]
    )
    out_lyr.SyncToDisk()
